...
const ReqModal = ({ closeModal, quantity, setQuantity, submit, product, setProduct }: {
  closeModal: () => void,
  quantity: string,
    ...
    (
      async () => {
        try {
          ...
          setProducts(data.inventory.map((item: { product: string }) => item.product))
                ...
                <View className='w-9/12 px-2 rounded-lg mx-3 border'>
                  ...
                  >
                    {products.map((item, idx) => {
                      return (
                        <Picker.Item label={item} value={item} key={`${item}-${idx}`} />
                      );
                    })}
                  </Picker>
                </View>

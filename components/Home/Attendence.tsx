...
const Attendence = () => {
	...
	const markAttendence = async () => {
		...
		try {
			...
			console.log(body);
			const { data } = await axios.post(`${API_URL}/attendance/mark`, body);
			console.log(data);
	...
	return (
		<View className="flex w-full justify-center items-center p-3">
			<TouchableOpacity
				className="w-full bg-[#ff8731] rounded-lg"
				onPress={markAttendence}
			>
				<View>
					{loading ? (
						<View className="flex flex-row justify-center items-center">
							<Text className="text-xl text-center text-white py-4 font-semibold mr-1">
								{" "}
								Loading...{" "}
							</Text>
						</View>

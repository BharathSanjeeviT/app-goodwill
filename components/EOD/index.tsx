...
import { Text, ScrollView, TouchableOpacity, View, TextInput } from "react-native"
...
const EODComponent = () => {
  ...
  useEffect(() => {
    const initUpdatedProducts: Array<{ product: string, quantity: number }> = data.map(({ product, quantity }) => ({ product, quantity }));
  ...
  return (
    <View className="bg-[#ede3da] min-h-[100vh] pt-36">
      ...
      <View
        className="absolute top-20 left-0 w-full py-3 px-3 flex flex-row items-center justify-center z-10"
      >
